...
CREATE OR REPLACE PROCEDURE update_custo_info_library
   (col_in VARCHAR2 , aux_doppel_in NUMBER, new_val_in VARCHAR2)
IS
BEGIN
    case upper(col_in)
        when 'NAME' THEN 
--            dbms_output.put_line('> customer');
            UPDATE customer set CUSTOMERADDRESS = new_val_in
            WHERE customerID = aux_doppel_in ;
        when 'ADDRESS' THEN 
--            dbms_output.put_line('> address');
            UPDATE customer set CUSTOMERADDRESS = new_val_in
            WHERE customerID = aux_doppel_in ;
        when 'NEWPASS' THEN 
--            dbms_output.put_line('> newpass');
            UPDATE customer set CUSTOMERADDRESS = new_val_in
            WHERE customerID = aux_doppel_in ;
        else
             dbms_output.put_line ('> else');
    end CASE;
END update_custo_info_library;
/*Modify Customer DATA*/
DECLARE
  col_in VARCHAR2(1000); 
  aux_doppel_in NUMBER;
  new_val_in VARCHAR2(1000);
BEGIN
    aux_doppel_in := &ID;
    col_in := '&Column';
    new_val_in :='&NEW_NAME';
    update_custo_info_library(col_in, aux_doppel_in, new_val_in);
END;
/
-----PROCEDURE_FOR_UPDATE_INFO_EMPLOYEE-------
CREATE OR REPLACE PROCEDURE update_employee_info_library
   (col_in VARCHAR2 , aux_doppel_in NUMBER, new_val_in VARCHAR2)
IS
BEGIN
   CASE UPPER(col_in)
         WHEN 'NAME' THEN 
             UPDATE employee SET NAME = new_val_in
             WHERE employeeid = aux_doppel_in;
         WHEN 'EMPLOYEEADDRESS' THEN
             UPDATE employee SET EMPLOYEEADDRESS = new_val_in
             WHERE employeeid = aux_doppel_in;
         WHEN 'BRANCHNAME' THEN 
             UPDATE employee SET BRANCHNAME = new_val_in
             WHERE employeeid = aux_doppel_in;
         ELSE
           DBMS_OUTPUT.PUT_LINE('ELSE');
   END CASE;           
END update_employee_info_library;
/*Test PROCEDURE_FOR_UPDATE_INFO_EMPLOYE*/
DECLARE
  col_in VARCHAR2(1000); 
  aux_doppel_in NUMBER;
  new_val_in VARCHAR2(1000);
BEGIN
    ...
    col_in := '&Column';
    new_val_in :='&NEW_NAME';
    update_employee_info_library(col_in, aux_doppel_in, new_val_in);
END;
/
--------ADD_TRIGGERS-FOR-CUSTOME-AND-EMPLOYEE----------------------
...
BEGIN
  ...
  VALUES(:new.CARDNUMBER,'A',0);
   DBMS_OUTPUT.PUT_LINE('Card created');
END;
-------SHOW_ALL_LIBRARY_FROM_BOOK__CDS--------
CREATE OR REPLACE PROCEDURE show_all_library (all_in VARCHAR2)
IS 
...
BEGIN
  IF all_in = 'book' THEN
         OPEN c_cursor FOR SELECT * FROM book ; 
             DBMS_OUTPUT.PUT_LINE('ISBN ID STATE AVALAB DEBYCOST LOSTCOST ADDRESS' );
             DBMS_OUTPUT.PUT_LINE('-----------------------------------------------'); 
         LOOP
           FETCH c_cursor INTO xbooks;
           EXIT WHEN c_cursor%notfound;
           DBMS_OUTPUT.PUT_LINE(xbooks.isbn|| ' ' ||xbooks.bookID|| ' ' ||xbooks.state|| ' ' ||xbooks.avalab|| ' ' || xbooks.debycost
           || ' ' ||xbooks.lostcost|| ' ' ||xbooks.address);
         END LOOP;
      CLOSE c_cursor;
  ELSIF all_in = 'cds' THEN
         OPEN c_cursor FOR SELECT * FROM cds;
            DBMS_OUTPUT.PUT_LINE('TITEL YEAR CDID STATE AVALAB DEBYCOST LOSTCOST ADDRESS' );
            DBMS_OUTPUT.PUT_LINE('-----------------------------------------------'); 
        
        LOOP
           FETCH c_cursor INTO xcds;
           EXIT WHEN c_cursor%notfound;
           DBMS_OUTPUT.PUT_LINE(xcds.titel|| ' ' ||xcds.year|| ' ' ||xcds.cdid ||' '||xcds.state|| ' ' ||xcds.avalab|| ' ' || xcds.debycost
           || ' ' ||xcds.lostcost|| ' ' ||xcds.address);
        END LOOP;
       CLOSE c_cursor;
  ELSE       
    DBMS_OUTPUT.PUT_LINE('The type incorrect ' );
  END IF;     
END show_all_library;

DECLARE 
all_in VARCHAR2(8);
BEGIN
 all_in :='&CD_OR_BOOK';
 show_all_library(all_in);
END;
------PROCEDURE--HANDEL_RETURN_LIBRARY------
CREATE OR REPLACE PROCEDURE handel_return_library (auxitem_in VARCHAR2)
IS
    auxreturned NUMBER;
    auxbook     NUMBER;
    auxcds      NUMBER;
BEGIN
   SELECT COUNT(*) INTO auxreturned 
   FROM rent WHERE itemid = auxitem_in;
   
   SELECT COUNT(*) INTO auxbook
   FROM book WHERE bookid = auxitem_in;
   
   SELECT COUNT(*) INTO auxcds
   FROM cds WHERE cdid = auxitem_in;
   
 IF auxreturned > 0 THEN
       DELETE FROM rent 
       WHERE itemid = auxitem_in;
           
       IF auxbook > 0 THEN 
          UPDATE book SET avalab = 'A'
          WHERE bookid = auxitem_in;
          DBMS_OUTPUT.PUT_LINE('The book ' ||auxitem_in|| ' is avaible');
          
       ELSIF auxcds > 0 THEN 
          UPDATE cds SET avalab = 'A'
          WHERE cdid = auxitem_in;
          DBMS_OUTPUT.PUT_LINE('The CD ' ||auxitem_in|| ' is avaible');
 ...
 END IF;
     ...
     WHEN no_data_found THEN
      DBMS_OUTPUT.PUT_LINE('ItemID uncorrect');  
END handel_return_library;

DECLARE
auxitem_in VARCHAR2(8);
BEGIN
auxitem_in :='&Item_id_to_return';
handel_return_library(auxitem_in);
END;
-----
